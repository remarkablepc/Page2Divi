...
## Highlights in v1.00 (Released July 2026 for the 250th of USA <img src="https://flagcdn.com/us.svg" width="18" alt="USA Flag">)
...
## What's new in the next local version bump

These notes are cumulative until the next publish and should stay in sync with `CHANGELOG.md` under `Unreleased`:
...
### Local build artifacts and paths

These are the current local packaging names and paths described by the build scripts:

- **Windows latest launcher:** `release/dist/latest/Page2Divi.exe`
- **Windows latest package folder:** `release/dist/latest/`
- **Windows versioned package folder:** `release/dist/Page2Divi-vX.XX/`
- **Windows versioned executable:** `release/dist/Page2Divi-vX.XX/Page2Divi-vX.XX.exe`
- **macOS versioned package folder:** `release/dist/Page2Divi-vX.XX/`
- **macOS versioned app bundle:** `release/dist/Page2Divi-vX.XX/Page2Divi.app`
- **macOS versioned archive:** `release/dist/Page2Divi-vX.XX-macos-universal2.zip`
- **Latest build marker:** `release/dist/LATEST.txt`
...
- **Tooltip Updates:** Redesigned and corrected the tooltips for "Follow Site Menu" and "Page Discovery" to match their actual, non-overlapping functions.
- **Clearer Multi-Page Run Feedback:** Follow-menu and page-discovery runs now provide more useful crawl feedback in the Activity Log.
- **Single and Dual Progress Bars for Runs:** Single and Multi-page batches now show a dedicated progress bar. Multi-page runs also have an overall ETA above the current-page progress bar, while single-page runs keep the simpler one-bar layout. The window also resizes when that extra batch row appears so the bottom controls remain visible.
...
3. Pick an input – URL, URL list, sitemap, pasted HTML, local file, or local folder.  
4. Pick **Divi 4** or **Divi 5** as the target.  
5. Click **Preview** (parser dry-run) or **Convert** (writes `.Divi4.json` / `.Divi5.json` + media).  
6. In Divi, use **Portability → Import** on the resulting `.Divi4.json` or `.Divi5.json` file.  
...
| --- | --- | --- |
| **Windows x64** | `release/dist/latest/Page2Divi.exe` and `release/dist/Page2Divi-vX.XX/Page2Divi-vX.XX.exe` | Local Windows builds keep a stable latest launcher plus a versioned EXE in the versioned package folder. No installer, no admin required. |
| **macOS (universal2)** | `release/dist/Page2Divi-vX.XX-macos-universal2.zip` | Local macOS builds emit a universal2 zip containing `Page2Divi.app`, with the unpacked app bundle staged under the matching versioned package folder. |
...
> It builds from the same source tree and the code paths it touches are cross-platform, but Mac-specific behavior (Gatekeeper, file dialogs, Tk on Retina, etc.) has not been verified end-to-end.
